...
function PAGE:Load(contentFrame, itemData)
	...
	PAGE.ItemData.type = "lua"
	PAGE.ItemData.lua = "" --Called when the user uses this item
	...
	textPanel.Paint = function(s , w , h)
		draw.SimpleText(BU3.Lang.Get("LUA_EDITOR"), BU3.UI.Fonts["large_bold"], w/2, h/2, Color(255,255,255,20),1 ,1)
	end

	local stepCounter = BU3.UI.Elements.CreateStepCounter(3, self.mirrorPanel)
	stepCounter.selectedStage = 1
	stepCounter:SetPos(contentFrame:GetWide()/2 - 200 + 45, 80)
	...
	descEntry:SetSize(300, 235 - 40 - 5 - 40 - 40 + 40 + 20)
	descEntry:SetText(PAGE.ItemData.desc)
	descEntry.OnValueChange = function(s , val)
		PAGE.ItemData.desc = val
	end

	--local weaponClassEntry = BU3.UI.Elements.CreateTextEntry(BU3.Lang.Get("ENTCLNAME"), self.pages, false, false)
	...
	local iconType = "Default Lua Icon"
	if PAGE.ItemData.iconIsModel then
		iconType = "Model"
	elseif PAGE.ItemData.iconID ~= "lua" then
		iconType = "Imgur ID"
	end
	...
	iconTypeSelect:AddChoice("Default Lua Icon")
	iconTypeSelect:AddChoice("Model")
	iconTypeSelect:AddChoice("Imgur ID")
	iconTypeSelect.OnSelect = function( panel, index, value )
		if value == "Default Lua Icon" then
			PAGE.ItemData.iconIsModel = false
			PAGE.ItemData.iconID = "lua"
			imgurIDEntry.canEdit = false
			previewImage:Remove()
			previewImage = BU3.UI.Elements.IconView("lua", iconViewStartColor, self.pages, true)
			previewImage:SetPos(320, 150)
			previewImage:SetSize(170, 170)
			previewImage.zoom = PAGE.ItemData.zoom
			previewImage.placeholder = false		
	...
	if PAGE.ItemData.iconID ~= "lua" then
	...
	giftText:SetPos(baseOffset + 235, 260 - 35 - 35)
...
------------------------
	...
	giftCheckBox:SetPos(baseOffset + 525 + 10,260 - 35 - 35)
...
------------------------
	...
	baseOffset = baseOffset + contentFrame:GetWide()

	--On the new page add the entry box as well as the create/update button
	local luaEntry = BU3.UI.Elements.CreateMultilineTextEntry(BU3.Lang.Get("ENTERLUAHERE"), self.pages, false)
	luaEntry:SetPos(baseOffset + 235 - 100, 150 + 30)
	luaEntry:SetSize(340 + 200, 445 - 30)
	if PAGE.ItemData.lua ~= nil then
		luaEntry:SetText(PAGE.ItemData.lua)
	end
	luaEntry.OnValueChange = function(s , val)
		PAGE.ItemData.lua = val
	end
	
 
	--Creates a pop up to add an item
	local createCaseButton = BU3.UI.Elements.CreateStandardButton(BU3.Lang.Get("CREATEUPLUA"), self.pages, function()
	...
	createCaseButton:SetPos(baseOffset + (contentFrame:GetWide()/2) - 178, 250 + 400 - 10)
end
...
--Register the page
BU3.UI.RegisterPage("luaeditor", PAGE)
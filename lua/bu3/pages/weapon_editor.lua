...
function PAGE:Load(contentFrame, itemData)
	...
	self.pages:SetSize(contentFrame:GetWide() * 2, contentFrame:GetTall()) --Multiple by five so we can move it to change slides
	...
	textPanel.Paint = function(s , w , h)
		draw.SimpleText(BU3.Lang.Get("WPN_EDITOR"), BU3.UI.Fonts["large_bold"], w/2, h/2, Color(255,255,255,20),1 ,1)
	end

	local stepCounter = BU3.UI.Elements.CreateStepCounter(2, self.mirrorPanel)
	...
	local weaponClassEntry = BU3.UI.Elements.CreateTextEntry(BU3.Lang.Get("WPN_CLASS_NAME"), self.pages, false, false)
	...
	local iconType = "Default Weapon Icon"
	if PAGE.ItemData.iconIsModel then
		iconType = "Model"
	elseif PAGE.ItemData.iconID ~= "weapon" then
		iconType = "Imgur ID"
	end
	...
	iconTypeSelect:AddChoice("Default Weapon Icon")
	iconTypeSelect:AddChoice("Model")
	iconTypeSelect:AddChoice("Imgur ID")
	iconTypeSelect.OnSelect = function( panel, index, value )
		if value == "Default Weapon Icon" then
			PAGE.ItemData.iconIsModel = false
			PAGE.ItemData.iconID = "weapon"
			imgurIDEntry.canEdit = false
			previewImage:Remove()
			previewImage = BU3.UI.Elements.IconView("weapon", iconViewStartColor, self.pages, true)
			previewImage:SetPos(320, 150)
			previewImage:SetSize(170, 170)
			previewImage.zoom = PAGE.ItemData.zoom
			previewImage.placeholder = false		
	...
	if PAGE.ItemData.iconID ~= "weapon" then
...
--------------------
	...
	local createCaseButton = BU3.UI.Elements.CreateStandardButton(BU3.Lang.Get("CREATEUPWPN"), self.pages, function()
...
--Register the page
BU3.UI.RegisterPage("weaponeditor", PAGE)
...
function PAGE:Load(contentFrame, itemData)
	...
	PAGE.ItemData.moneyAmount = ""
	PAGE.ItemData.type = "money"
	...
	textPanel.Paint = function(s , w , h)
		draw.SimpleText(BU3.Lang.Get("MONEY_EDITOR"), BU3.UI.Fonts["large_bold"], w/2, h/2, Color(255,255,255,20),1 ,1)
	end
	...
	local weaponClassEntry = BU3.UI.Elements.CreateTextEntry(BU3.Lang.Get("MONEY_AMOUNT"), self.pages, false, false)
	weaponClassEntry:SetPos(85, 340 + 20 + 40 + (235 - 40 - 5 - 40) + 20 - 40)
	weaponClassEntry:SetSize(300, 40)
	weaponClassEntry:SetText(PAGE.ItemData.moneyAmount)
	weaponClassEntry.OnValueChange = function(s , val)
		PAGE.ItemData.moneyAmount = val
	end
	...
	local iconType = "Default Money Icon"
	...
	iconTypeSelect:AddChoice("Default Money Icon")
	iconTypeSelect:AddChoice("Model")
	iconTypeSelect:AddChoice("Imgur ID")
	iconTypeSelect.OnSelect = function( panel, index, value )
		if value == "Default Money Icon" then
			PAGE.ItemData.iconIsModel = false
			PAGE.ItemData.iconID = "money"
			imgurIDEntry.canEdit = false
			previewImage:Remove()
			previewImage = BU3.UI.Elements.IconView("money", iconViewStartColor, self.pages, true)
			previewImage:SetPos(320, 150)
			previewImage:SetSize(170, 170)
			previewImage.zoom = PAGE.ItemData.zoom
			previewImage.placeholder = false		
	...
	if PAGE.ItemData.iconID ~= "money" then
...
------------------------
	...
	local createCaseButton = BU3.UI.Elements.CreateStandardButton(BU3.Lang.Get("CREATEUPMONEY"), self.pages, function()
...
--Register the page
BU3.UI.RegisterPage("moneyeditor", PAGE)
...
function PAGE:Load(contentFrame, itemData)
	...
	--Init some stuff
	self.ItemData.gift = true
	self.ItemData.name = ""
	self.ItemData.desc = ""
	self.ItemData.iconID = "points"
	self.ItemData.iconIsModel = false
	self.ItemData.rankRestricted = false
	self.ItemData.price = 0
	self.ItemData.zoom = 0
	self.ItemData.color = Color(255,255,255)
	self.ItemData.canBeBought = false
	self.ItemData.canBeSold = true
	self.ItemData.perm = false
	self.ItemData.ranks = {}
	self.ItemData.itemColorCode = 1 --Gray
	self.ItemData.pointsAmount = ""
	self.ItemData.premium = false
	self.ItemData.type = "points2"

	--Update the table to the correct info
	if isEditing then
		for k, v in pairs(itemData) do
			self.ItemData[k] = itemData[k]
		end
	...
	textPanel.Paint = function(s , w , h)
		draw.SimpleText(BU3.Lang.Get("PS2_EDITOR"), BU3.UI.Fonts["large_bold"], w/2, h/2, Color(255,255,255,20),1 ,1)
	end
	...
	local weaponClassEntry = BU3.UI.Elements.CreateTextEntry(BU3.Lang.Get("POINTS_AMOUNT"), self.pages, false, false)
	weaponClassEntry:SetPos(85, 340 + 20 + 40 + (235 - 40 - 5 - 40) + 20 - 40)
	weaponClassEntry:SetSize(300, 40)
	weaponClassEntry:SetText(PAGE.ItemData.pointsAmount)
	weaponClassEntry.OnValueChange = function(s , val)
		PAGE.ItemData.pointsAmount = val
	end
	...
	local iconType = "Default Points Icon"
	if PAGE.ItemData.iconIsModel then
		iconType = "Model"
	elseif PAGE.ItemData.iconID ~= "money" then
		iconType = "Imgur ID"
	end
	...
	iconTypeSelect:AddChoice("Default Points Icon")
	iconTypeSelect:AddChoice("Model")
	iconTypeSelect:AddChoice("Imgur ID")
	iconTypeSelect.OnSelect = function( panel, index, value )
		if value == "Default Points Icon" then
			PAGE.ItemData.iconIsModel = false
			PAGE.ItemData.iconID = "points"
			imgurIDEntry.canEdit = false
			previewImage:Remove()
			previewImage = BU3.UI.Elements.IconView("points", iconViewStartColor, self.pages, true)
			previewImage:SetPos(320, 150)
			previewImage:SetSize(170, 170)
			previewImage.zoom = PAGE.ItemData.zoom
			previewImage.placeholder = false		
	...
	if PAGE.ItemData.iconID ~= "points" then
	...
	zoomText:SetPos(430,  485 + 25 + 25 + 25 - 10 - 3)
	...
	zoom:SetPos(430, 485 + 25 + 25 + 25 + 25 + 25 - 25 - 3)
	zoom:SetSize(300, 16)
...
------------------------
	...
	local premiumCheckbox = BU3.UI.Elements.CreateCheckBox(false, self.pages)
	premiumCheckbox:SetPos(430 + 300 - 40, 485 + 25 + 25 + 25 + 25 + 25 - 1)
	premiumCheckbox:SetSize(40 * 1, 20 * 1)
	premiumCheckbox.state = PAGE.ItemData.premium
	premiumCheckbox.Think = function(s)
		PAGE.ItemData.premium = s.state
	end

	local premiumText = vgui.Create("DPanel", self.pages)
	premiumText:SetSize(200, 40)
	premiumText:SetPos(430, 485 + 25 + 25 + 25 + 25 + 25 - 1)
	premiumText.Paint = function(s , w , h)
		draw.SimpleText(BU3.Lang.Get("USE_PREMIUM_PTS"), BU3.UI.Fonts["small_bold"], 0, 0, Color(210,210,210,200),0 ,0)
	end	

	--Creates a pop up to add an item
	local createCaseButton = BU3.UI.Elements.CreateStandardButton(BU3.Lang.Get("CREATEUPPTS"), self.pages, function()
...
--Register the page
BU3.UI.RegisterPage("points2editor", PAGE)